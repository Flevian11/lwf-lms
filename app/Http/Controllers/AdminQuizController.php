...
class AdminQuizController extends Controller
{
    ...
    private function eligibleStudents(int $courseId, ?int $quizId = null){
        return User::query()
            ...
            ->when($quizId, function($q) use ($quizId) {
                // Exclude anyone who already has an allocation or attempt. Use direct
                // table subqueries rather than requiring inverse User relationships.
                $q->whereNotExists(function($sub) use ($quizId) {
                    $sub->selectRaw('1')
                        ->from('quiz_allocations')
                        ->whereColumn('quiz_allocations.user_id', 'users.id')
                        ->where('quiz_allocations.quiz_id', $quizId);
                })->whereNotExists(function($sub) use ($quizId) {
                    $sub->selectRaw('1')
                        ->from('quiz_attempts')
